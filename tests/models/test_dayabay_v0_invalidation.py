...
def test_dayabay_v0_invalidation():
    ...
    with open("output/tainted.txt", "w") as f:
        f.write("# parname\tntainted\tncalls\tevaltime\n")
...
def test_dayabay_v0_invalidation_mean():
    from pandas import read_csv

    fn = "output/tainted.txt"
    df = read_csv(fn, delimiter="\t", skiprows=1, names=["name", "ntainted", "evaltime"])
